...
- [x] 11-03-PLAN.md — read-only reprint buttons + printer-configured gate (D-04/05/06) + `onPrint={handlePrint}` history-detail wiring (Pitfall 1) (HIST-11)
- [x] 11-04-PLAN.md — activate `h_export` button: `buildCsv(visible)` → native `save()` → `writeTextFile()`, cancel/error/empty handling, D-14 filename (HIST-12)
...
- [x] 12-01-PLAN.md — D-06 `normalizeOrder` fallback-total + percent-discount regression tests (verify-only backfill)
- [x] 12-02-PLAN.md — D-09 HIST-06 traceability fix in 07-VERIFICATION.md + 07-04-SUMMARY.md (doc-only)
- [x] 12-03-PLAN.md — D-01…D-04 lift History selection into a session-only `historySelection` store slice + setScreen reset + test-mock fix (the one code change)
