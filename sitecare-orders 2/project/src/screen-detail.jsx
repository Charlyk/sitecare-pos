...
function OrderDetailScreen({ order, lang, onBack, onAdvance, onPrint }) {
  ...
  const src = window.sourceMeta(order.source, t);
  const typ = window.typeMeta(order.type, t);
  const st = window.stateMeta(order.state, t);
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--sc-muted-foreground)', marginTop: 4 }}>
              {window.orderTimeLabel(order.placedAt)} · {elapsed} {t('min')} {t('elapsed').toLowerCase()}
            </div>
          </div>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          {order.items.map((it, i) => (
            <div key={i} style={{ padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: i < order.items.length - 1 ? '1px solid hsl(120 10% 94%)' : 'none' }}>
              ...
              <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--sc-muted-foreground)' }}>{window.formatRON(it.price)}</div>
              <div style={{ fontWeight: 800, fontSize: 14, minWidth: 72, textAlign: 'right' }}>{window.formatRON(it.price * it.qty)}</div>
            </div>
          ))}
          <div style={{ padding: 18, background: '#fbf6ea' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}>
              <span>{t('subtotal')}</span><span style={{ fontWeight: 600 }}>{window.formatRON(order.subtotal)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}>
              <span>{t('tax')}</span><span style={{ fontWeight: 600 }}>{window.formatRON(order.tax)}</span>
            </div>
            {order.deliveryFee > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}><span>{t('delivery_fee')}</span><span style={{ fontWeight: 600 }}>{window.formatRON(order.deliveryFee)}</span></div>}
            {order.tip > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}><span>{t('tip')}</span><span style={{ fontWeight: 600 }}>{window.formatRON(order.tip)}</span></div>}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', paddingTop: 8, marginTop: 8, borderTop: '1px dashed hsl(120 10% 82%)' }}>
              ...
              <span style={{ fontWeight: 900, fontSize: 26, letterSpacing: '-0.02em', color: 'var(--sc-primary)' }}>{window.formatRON(order.total)}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <span>{window.orderTimeLabel(order.placedAt)}</span>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span>{window.typeMeta(order.type, window.useT(lang)).label.toUpperCase()}{order.table ? ` · MASA ${order.table}` : ''}</span>
