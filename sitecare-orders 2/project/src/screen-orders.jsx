    ...
    <div className="card shadow" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, border: order.state === 'new' ? '1.5px solid hsl(0 53% 58% / 0.4)' : '1px solid hsl(120 10% 90%)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 11, color: 'var(--sc-muted-foreground)', fontWeight: 600 }}>{window.orderTimeLabel(order.placedAt)}</div>
      ...
      <div style={{ borderTop: '1px dashed hsl(120 10% 88%)', paddingTop: 10, display: 'flex', flexDirection: 'column', gap: 4 }}>
        {order.items.slice(0, 3).map((it, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
            ...
            <span style={{ color: 'var(--sc-muted-foreground)' }}>{window.formatRON(it.qty * it.price)}</span>
          </div>
        ))}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 8, borderTop: '1px solid hsl(120 10% 92%)' }}>
        <div>
          ...
          <div style={{ fontWeight: 900, fontSize: 18, letterSpacing: '-0.02em' }}>{window.formatRON(order.total)}</div>
        </div>
...
function OrdersScreen({ orders, lang, onOpen, onAdvance, onPrint }) {
  ...
  const [typeFilter, setTypeFilter] = useStateOrders('all');
  ...
  const stats = [
    { label: lang === 'ro' ? 'Venit azi' : 'Today revenue', value: window.formatRON(todayRevenue), icon: 'ron', tint: 'sage' },
